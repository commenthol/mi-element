...
function cookieSerialize(name, value, options) {
  ...
  return (
    !isNaN(maxAge - 0) && isFinite(maxAge) && parts.push(`Max-Age=${maxAge}`),
    domain && fieldContentRegExp.test(domain) && parts.push(`Domain=${domain}`),
    path && fieldContentRegExp.test(path) && parts.push(`Path=${path}`),
    isNaN(new Date(expires).getTime()) ||
      parts.push(`Expires=${new Date(expires).toUTCString()}`),
    httpOnly && parts.push('HttpOnly'),
    secure && parts.push('Secure'),
    sameSite && parts.push(`SameSite=${sameSite}`),
    parts.join('; ')
  )
}
