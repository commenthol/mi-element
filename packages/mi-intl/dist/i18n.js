...
const format = cached(),
  isBrowser = globalThis?.navigator?.language && globalThis?.document,
  defaultOptions = {
    supportedLngs: [],
    version: '',
    ns: ['translations'],
...
const uniq = (arr) => [...new Set(arr.filter(Boolean))],
  mainLng = (lng) => (lng || '').split('-', 1)[0],
  reduceSupportedLangs = (lng, supportedLngs, browserLngs = []) =>
    uniq(
      [lng, ...browserLngs]
        .filter(Boolean)
        .reduce(
          (curr, nextLng) => (curr.push(nextLng, mainLng(nextLng)), curr),
          []
        )
    ).reduce(
      (curr, nextLng) => (
        supportedLngs.includes(nextLng) && curr.push(nextLng),
        curr
      ),
      []
    )
...
class I18n {
  constructor(options) {
    const {
      ...
      lng: lng,
      defaultNs: defaultNs,
      fallbackLng: fallbackLng = supportedLngs[0],
      resources: resources = {},
      debug: debug,
      ...opts
    } = {
  ...
  t(label, values = {}) {
    const {
      ...
      { lng: lng = language, ns: ns = defaultNs, ...msgValues } = values,
      message =
  ...
  resetUserLanguage() {
    ...
    ;(log.debug('reset cookie'),
      (document.cookie = cookieSerialize(name, '', {
        ...opts,
        expires: 0
      })),
      this._setLanguage())
  ...
  _getSettings() {
    const { name: name } = this.options.cookie,
      cookieLng = cookieParse(document.cookie)[name],
      browserLng = globalThis?.navigator?.language
    return (
  ...
  _setCookie() {
    if (!isBrowser) return
    const [cookieLng, browserLng] = this._getSettings(),
      { userLng: language = '' } = this
    if (cookieLng === language) return
    let expires,
      value = language
    ;[browserLng, mainLng(browserLng)].includes(language) &&
      ((expires = 0), (value = ''))
    const { name: name, ...opts } = this.options.cookie
    ;(log.debug('_setCookie: %s:%s', 0 === expires ? 'reset' : 'set', language),
      (document.cookie = cookieSerialize(name, value, {
        ...opts,
        expires: expires
      })))
  }
  _setLanguage(lng) {
    const _lng = lng || this.getUserLanguage(),
      { supportedLngs: supportedLngs } = this,
      browserLngs = globalThis?.navigator?.languages || [],
      variants = reduceSupportedLangs(_lng, supportedLngs, browserLngs)
    if (variants.length) ((this.userLng = _lng), (this.lng = variants[0]))
    else {
  ...
  async loadLanguages(lngs = [], ns = []) {
    const _lngs = uniq(lngs),
      _ns = uniq([...ns, this.defaultNs])
    if (!_lngs.length || !_ns.length)
      return (log.error("can't load without languages or namespaces"), [])
    const loaders = []
    for (const lng of _lngs)
      for (const ns of _ns)
        this.resources[lng]?.[ns] ||
          loaders.push(
            this._load({
              lng: lng,
              ns: ns
  ...
  async _load({ lng: lng, ns: ns }) {
    const { version: version } = this.options,
      url = format(this.options.localesPath, {
        lng: lng,
        ns: ns,
        version: version
      }),
